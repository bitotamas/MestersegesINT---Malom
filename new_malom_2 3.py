...
def choose_piece_to_remove(opponent_pieces, mill_pieces):
    """
    Kiválaszt egy bábut eltávolításra.
    
    Először olyan bábut választ, amely nem része malomnak. 
    Ha minden bábu malomban van, akkor egy véletlenszerű bábut választ.

    Args:
        opponent_pieces (list): Az ellenfél bábuinak listája.
        mill_pieces (list): Az ellenfél malomban lévő bábui.

    Returns:
        tuple: A kiválasztott bábu pozíciója.
    """
    non_mill_pieces = [p for p in opponent_pieces if p not in mill_pieces]
    if non_mill_pieces:
        # Ha van nem-malom bábu, válaszd azt.
        return random.choice(non_mill_pieces)
    else:
        # Ha minden bábu malomban van, válassz egy véletlenszerűt.
        return random.choice(opponent_pieces)

def can_move_out_of_mill(piece, player_pieces, occupied_positions, neighbors):
    """
    Ellenőrzi, hogy egy bábu kiléphet-e a malomból.

    Args:
        piece (tuple): A bábu jelenlegi pozíciója.
        player_pieces (list): A játékos bábui.
        occupied_positions (list): Az összes foglalt pozíció.
        neighbors (dict): Szomszédsági lista.

    Returns:
        bool: True, ha kiléphet, False, ha nem.
    """
    if not is_in_mill(piece, player_pieces):
        return True  # Ha nem malomban van, szabadon mozoghat.
    
    for neighbor in neighbors.get(piece, []):
        if neighbor not in occupied_positions:
            return True  # Ha van szabad szomszéd, léphet.
    
    return False  # Nincs szabad szomszéd, nem léphet.
...
def main():
    ...
    while True:
        ...
        # Játékosok idejének megjelenítése
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if remove_phase:
                    ...
                    for position in (non_mill_pieces if non_mill_pieces else opponent_pieces):
                ...
                # Korongok elhelyezése (placing fázis) - malom nem történhet
                ...
                # Korongok mozgatása (moving fázis)
                    ...
                    if selected_piece is None:
                        ...
                        # Ha a játékos már kijelölt egy bábút, nézzük meg, hova kattintott
                        for neighbor in neighbors[selected_piece]:
                            if pygame.Rect(neighbor[0] - 20, neighbor[1] - 20, 40, 40).collidepoint(pos) and neighbor not in occupied_positions:
                                if is_in_mill(selected_piece, white_pieces) and can_move_out_of_mill(selected_piece, white_pieces):
                                    move_piece(neighbor)  # Kilépés a malomból, ha az lehetséges
                                else:
                                    move_piece(neighbor)
                                selected_piece = None
                                break
        ...
        # Játékosok korongjainak megjelenítése
        ...
        current_player_text = player_name if current_player == "fehér" else "Fekete"
        draw_text(f"Jelenlegi játékos: {current_player_text}", (300, 50), BLACK)

        pygame.display.flip()
        clock.tick(30)
def can_move_out_of_mill(piece, player_pieces, occupied_positions, neighbors):
    # Ellenőrizzük, hogy a bábú malomban van-e és ki tud-e lépni
    if piece in player_pieces:
        for neighbor in neighbors[piece]:
            if neighbor not in occupied_positions:
                return True  # Ha a bábú mozdítható, akkor kiléphet a malomból
    return False
